...
from checker_file import complete_process
...
class Application(tk.Frame):
    ...
    def create_pager(self):
        ...
        self.pager = tk.Frame(self.main_frame, bg=self.bg_main_color, height=50, pady=20)
        self.pager.columnconfigure([0, 1, 2], weight=1)
        self.pager.grid(column=0, row=1, sticky=tk.EW)

        self.prev_button = tk.Button(
            self.pager, text="Anterior", font=("Arial", 16),
            bg=self.bg_button_main_color, fg='white', relief=tk.FLAT,
            state=tk.DISABLED, cursor="hand2", command=lambda: self.change_page(-1)
        )
        self.prev_button.grid(column=0, row=0)

        self.page_number = tk.Label(
            self.pager, bg=self.bg_main_color, fg=self.bg_font_color,
            font=("Arial", 18),
        ...
        self.next_button = tk.Button(
            self.pager, text="Siguiente", font=("Arial", 16),
            bg=self.bg_button_main_color, fg='white', relief=tk.FLAT,
            state=tk.DISABLED, cursor="hand2", command=lambda: self.change_page(1)
        )
    ...
    def etapa_1(self):
        ...
        self.content.config(text="Los archivos de este tipo siguen el siguiente formato de nombre:\nventas_cbte_YYYYMM.txt\nEjemplo: ventas_cbte202210.txt")
        self.update_label()
        self.button.config(
            text="Seleccionar archivo",
            command=lambda: self.explorar_archivo(0)
        )
    ...
    def etapa_2(self):
        ...
        self.button.config(
            text="Seleccionar archivo",
            command=lambda: self.explorar_archivo(1)
        )

    def etapa_3(self):
        """Tercera etapa: Selección de la carpeta de destino."""
        self.title.config(text="Seleccionar carpeta de destino")
        self.content.config(text="Seleccione la carpeta en la que se guardarán los archivos finales: el archivo modificado y el reporte en formato .json.")
        self.button.config(
            text="Seleccionar carpeta",
            command=lambda: self.explorar_archivo(2)
        )

    def etapa_4(self):
        """Cuarta etapa: Confirmación y ejecución del proceso."""
        self.title.config(text="Confirmar y ejecutar proceso")
        self.content.config(text="Ejecute el proceso una vez seleccionados los archivos correspondientes y la carpeta destino.")
    ...
    def ejecutar_proceso(self):
        ...
        if ventas == self.archivo_default_value or alicuota == self.archivo_default_value or destino == self.carpeta_default_value:
            messagebox.showerror("Error", "Debe seleccionar ambos archivos y la carpeta de destino.")
            return

        try:
            resultado, message = complete_process(
